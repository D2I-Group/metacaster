...
class PastDecomposableMixing(nn.Module):
    ...
    def forward(self, x_list):
        ...
        for ori, out_season, out_trend, length in zip(
            x_list, out_season_list, out_trend_list, length_list, strict=False
        ):
...
class TimeMixerModel(nn.Module):
    ...
    def future_multi_mixing(self, b, enc_out_list, x_list):
        ...
        if self.channel_independence:
            x_list = x_list[0]
            for i, enc_out in zip(range(len(x_list)), enc_out_list, strict=False):
            ...
            for i, enc_out, out_res in zip(
                range(len(x_list[0])), enc_out_list, x_list[1], strict=False
            ):
                dec_out = self.predict_layers[i](enc_out.permute(0, 2, 1)).permute(
                    0, 2, 1
                )
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        x_mark_list = []
        if x_mark_enc is not None:
            for i, x, x_mark in zip(range(len(x_enc)), x_enc, x_mark_enc, strict=False):
                b, t, n = x.size()
            ...
            for i, x in zip(range(len(x_enc)), x_enc, strict=False):
                b, t, n = x.size()
                x = self.normalize_layers[i](x, "norm")
                if self.channel_independence:
                    x = x.permute(0, 2, 1).contiguous().reshape(b * n, t, 1)
        ...
        if x_mark_enc is not None:
            for _i, x, x_mark in zip(
                range(len(x_list[0])), x_list[0], x_mark_list, strict=False
            ):
                enc_out = self.enc_embedding(x, x_mark)
                enc_out_list.append(enc_out)
        else:
            for _i, x in zip(range(len(x_list[0])), x_list[0], strict=False):
